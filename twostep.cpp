		...
		const float noise)
{
	...
	// corresponding effect on the bias, too. TODO: evaluate if there is a
	// way to 'fixup' the estimate such that it favors a smaller gain
	// value for the beginning of the search.  The fixup portion of the
	// search does correct it, since there is a fair amount of that
	// information encoded in the gradiant of the cost function itself.
 ...
 //	estimate << Matrix<double, 3, 1>::Zero(), -Matrix<double, 3, 1>::Ones();
//	estimate.end<3>() = Vector3d::Zero();
...
#if PRINTF_DEBUGGING
	SelfAdjointEigenSolver<Matrix<double, 9, 9> > eig2(P_theta_theta_inv);
	std::cout << "eigenvalues: " << eig2.eigenvalues().transpose() << "\n";
	std::cout << "eigenvectors: \n" << eig2.eigenvectors() << "\n";
#endif
